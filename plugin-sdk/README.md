...
- **Host Environment**: QuickJS-based sandboxed runtime for executing plugin code. Plugins are JavaScript (or TypeScript compiled to JS) run inside this sandbox; the extension or app loads plugin source and passes it to the host (e.g. via `executePlugin(code)` or `window.tlsn.execCode(code)`).
